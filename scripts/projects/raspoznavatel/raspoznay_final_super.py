...
# Функция для исправления орфографии с сохранением структуры текста
def correct_spelling(text):
    spell = SpellChecker(language="ru")  # Устанавливаем язык проверки (русский)
    
    # Разделяем текст на строки, чтобы сохранить абзацы
    ...
    for line in lines:
        ...
        for word in words:
            # Находим наиболее вероятное исправление для слова
            corrected_word = spell.correction(word)
            
            if corrected_word:  # Если исправление найдено (не None)
                corrected_words.append(corrected_word if corrected_word else word)  # Добавляем исправленное слово. Используем исходное слово, если исправления нет.
            else:
                corrected_words.append(word)  # Если слово не нуждается в испqравлении, добавляем его как есть

        corrected_line = " ".join(corrected_words)  # Собираем исправленную строку
        corrected_lines.append(corrected_line)  # Добавляем исправленную строку в список

    # Соединяем все строки обратно в текст с сохранением структуры
