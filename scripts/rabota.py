class Job:
    ...
    def show_vacancies(self):
        if not self.vacancies:
            ...
            for vacancy in self.vacancies:
                print(
                    f"{vacancy['company']}: зарплата {vacancy['salary']} тыс., "
    ...
    def find_best_vacancy(self, exp, profession, min_salary):
        if profession != self.profession:
            print(
                f"Кажется, вы ищете вакансии {profession} "
                f"в разделе {self.profession}. Выберите другой раздел."
            )
            return
        ...
        if best_vacancy is not None:
            print(
                f"Наилучшая вакансия для вас в компании {best_vacancy['company']} "
