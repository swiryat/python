...
DB_PATH = r"C:\Users\swer\JavaRush_Articles\articles_ns.db"  # Путь к базе данных
...
def process_file(file_path):
    ...
    try:
        ...
        if title and file_path and category:
            ...
            print(f"❌ Пропущен файл с некорректными данными: {file_path}")
...
def db_worker():
    ...
    while True:
        try:
            ...
            if query and params:
                print(f"Вставка в БД: {params}")  # Логгируем запрос перед вставкой
                cursor.execute(query, params)
                conn.commit()
            else:
                print(f"❌ Пропущен запрос: {query} с параметрами {params}")
        except Exception as e:
...
def insert_article(connection, title, filepath, category):
    try:
        cursor = connection.cursor()
        # Проверяем, что переданные значения не None
        if None in [title, filepath, category]:
            raise ValueError("Одно из значений для вставки равно None")

        cursor.execute("INSERT INTO articles (title, filepath, category) VALUES (?, ?, ?)", (title, filepath, category))
        connection.commit()
        print(f"Вставка в БД: {title}, {filepath}, {category}")
    except Exception as e:
        print(f"Ошибка при вставке в базу данных: {e}")        
