...
def convert_to_black_and_white(image):
    ...
    _, black_and_white = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)  # Бинаризация (черно-белое)
    return black_and_white

# Функция для извлечения текста из изображения
def extract_text_from_image(image):
    bw_image = convert_to_black_and_white(image)  # Преобразуем в черно-белое изображение
...
bw_screenshot = convert_to_black_and_white(screenshot)
