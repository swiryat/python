...
model = SimpleCNN().to(device)  # Переносим модель на GPU
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)

# Создание экземпляра SummaryWriter для TensorBoard
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        images, labels = images.to(device), labels.to(device)  # Переносим данные на GPU
    ...
    # Логируем потери и точность в TensorBoard
...
# Загрузка модели
model = SimpleCNN().to(device)  # Создание новой модели и перенос на устройство
model.load_state_dict(torch.load('model.pth', weights_only=True))  # Загрузка параметров с безопасностью
...
# Здесь можно повторить цикл для логирования, если это необходимо, например, для дообучения
# Убедитесь, что `writer` открыт, если хотите продолжить логирование

# Закрываем writer, если он открыт
writer.close()
